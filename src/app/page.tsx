    ...
    return (
        <div className="flex" id="mainDiv">
            <main className="py-6 px-0 scroll-smooth md:ml-72 w-full bg-gradient-to-br from-blue-50 to-white min-h-screen rounded transition-colors duration-300">
                {/* Hero */}
                <section id="home" className="p-6 md:p-12 flex flex-col md:flex-row items-center bg-gradient-to-r from-slate-900 to-blue-900 m-3 rounded-2xl shadow-lg">
                ...
                <section id="skills" className="p-6 md:p-12 bg-white m-3 rounded-2xl shadow">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <SkillBar label={<span className="flex items-center gap-2"><FaHtml5 />HTML</span>} percentage={95} color="bg-blue-500" />
                        <SkillBar label={<span className="flex items-center gap-2"><FaCss3Alt />CSS</span>} percentage={80} color="bg-green-500" />
                        <SkillBar label={<span className="flex items-center gap-2"><SiTailwindcss />Tailwind CSS & <FaBootstrap />Bootstrap</span>} percentage={90} color="bg-purple-500" />
                        <SkillBar label={<span className="flex items-center gap-2"><FaJs />JavaScript</span>} percentage={80} color="bg-yellow-500" />
                        <SkillBar label={<span className="flex items-center gap-2"><FaReact />React</span>} percentage={80} color="bg-pink-500" />
                        <SkillBar label={<span className="flex items-center gap-2"><FaPhp />PHP</span>} percentage={90} color="bg-red-500" />
                        <SkillBar label={<span className="flex items-center gap-2"><SiSharp />C#</span>} percentage={70} color="bg-indigo-500" />
                        <SkillBar label={<span className="flex items-center gap-2"><FaPython />Python</span>} percentage={50} color="bg-orange-500" />
                        <SkillBar label={<span className="flex items-center gap-2"><FaLaravel />Laravel</span>} percentage={70} color="bg-red-900" />
                    </div>
                </section>

                {/* About Me */}
                <section id="about" className="min-h-[20vh] p-6 md:p-12 bg-gradient-to-r from-blue-900 to-slate-900 m-3 rounded-2xl shadow-lg">
